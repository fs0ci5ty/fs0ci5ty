<div align="center">

# Fsociety

**Offensive Security Research & Tooling**

![](https://img.shields.io/badge/status-active-0d0d0d?style=flat-square&labelColor=0d0d0d&color=0d0d0d)
![](https://img.shields.io/badge/focus-red_team-CC0000?style=flat-square&labelColor=0d0d0d&color=CC0000)
![](https://img.shields.io/badge/type-independent_research-00FF41?style=flat-square&labelColor=0d0d0d&color=CC0000)


![](https://komarev.com/ghpvc/?username=fs0ci5ty&style=flat-square&labelColor=0d0d0d&color=00FF41&label=profile+views)

</div>

---

Fsociety is an independent security research group publishing offensive tools, exploits, and automation scripts. Work spans red team operations, vulnerability research, and custom tooling — released for educational and authorized testing purposes only.

---

**Stack**

![Python](https://img.shields.io/badge/Python-0d0d0d?style=flat-square&logo=python&logoColor=00FF41)
![C](https://img.shields.io/badge/C-0d0d0d?style=flat-square&logo=c&logoColor=00FF41)
![Bash](https://img.shields.io/badge/Bash-0d0d0d?style=flat-square&logo=gnubash&logoColor=00FF41)
![PowerShell](https://img.shields.io/badge/PowerShell-0d0d0d?style=flat-square&logo=powershell&logoColor=00FF41)
![Kali Linux](https://img.shields.io/badge/Kali_Linux-0d0d0d?style=flat-square&logo=kalilinux&logoColor=00FF41)
![Burp Suite](https://img.shields.io/badge/Burp_Suite-0d0d0d?style=flat-square&logo=portswigger&logoColor=00FF41)

---

**Repositories**

| Repository | Description |
|---|---|
| `fsociety-toolkit` | Core offensive toolset for red team engagements |
| `recon-engine` | Automated reconnaissance and OSINT framework |
| `evasion-kit` | Endpoint defense bypass research |
| `ctf-archive` | CTF write-ups with full technical documentation |
| `script-vault` | Security automation and utility scripts |

---

<div align="center">

`github.com/fs0ci5ty`

*All tools are for educational purposes and authorized use only.*

</div>
